...
PlayerData::PlayerData(std::uint64_t id, const std::string_view& name, const std::string_view& email, const std::string_view& password, const std::string_view& country)
	: id(id), name(name), email(email), password(password), country(country)
{

}

std::uint64_t PlayerData::GetUserID() const {
	return id;
}

std::uint64_t PlayerData::GetProfileID() const {
	return id;
}